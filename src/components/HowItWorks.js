...
function HowItWorks() {
   const containerVariants = {
    hidden: {}, // No specific styles for the container itself when hidden, relies on children's states
    visible: {
      transition: {
        staggerChildren: 0.2, // Time difference between the start of each child's animation
  ...
  const cardVariants = {
    hidden: { opacity: 0, y: 30 }, // Start transparent and 30px down
    visible: { 
      opacity: 1, 
      y: 0, 
      transition: { duration: 0.5, ease: 'easeOut' } // Animate to opaque, original y position
    }
  }
  return (
    <motion.section 
    id="how-it-works" className="py-16 px-4 sm:px-6 lg:px-8 bg-tertiary text-text1"
    initial="hidden"
      whileInView="visible"
    ...
    >
      <div className="max-w-screen-xl mx-auto">
        ...
        initial={{ opacity: 0, y: -20 }} // Starts transparent and 20px up
          whileInView={{ opacity: 1, y: 0 }} // Animates to opaque and original y position
        ...
        {/* first section */}
          <motion.div 
          className="flex flex-col items-center text-center p-6 rounded-lg bg-secondary shadow-md"
